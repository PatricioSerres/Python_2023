import PySimpleGUI as sg
from PIL import Image
import base64,json,os
import json

def modificar_Json(dicc):                                       #recibe el usuario actual
    aux = []
    dir_json = os.path.join('archivos',"user.json")
    jsonData =open(dir_json,'r+')
    datos = json.load(jsonData)
    i = 0
    for line in datos:
        if (line['nick'] == dicc['nick']):                      #compara el nick de los usuarios cargados con el actual
                jsonData.close()
                jsonData = open(dir_json,'w')
                print(datos)
                datos[i] = dicc
                print(datos)
                json.dump(datos, jsonData)           
                break
        i += 1
    jsonData.close()    




def main (dicc):
        avatar = b''
        fondo = background_color=('white')

        columna1 = [
            #Texto de registrarse
            [sg.Text('Editar perfil',background_color=("white"),text_color=('Black'))],
            #Input de nick
            [sg.Text('nick',background_color=("white"),text_color=('Black'))],[sg.Input(dicc['nick'],size=(20,5),font=('calibri',10),key="nick",text_color=('#717171'),disabled=True)],
            #Input de nombre      
            [sg.Text('Nombre',background_color=("white"),text_color=('Black'))],[sg.Input(dicc['Nombre'],size=(20,5),font=('calibri',10),key='nombre',text_color=('#717171'))],
            #Input de edad
            [sg.Text('Edad',background_color=("white"),text_color=('Black'))],[sg.Input(dicc['Edad'],size=(8,5),key='edad',text_color=('#717171'))],
            #Genero
            [sg.Text('Género autopercibido',background_color=("white"),text_color=('Black'))],
            [sg.Input(dicc['Genero'],size=(8,5),disabled=False,key=('completar'),text_color=('#717171'))],
            # Guardar los datos
            [sg.Button('Guardar Datos',button_color=("green"),key='guardar',pad=(0,30)),sg.Button('Salir',key='salir',button_color=('#F53A3A'),pad=(20,0))]

        ]

        columna2 = [
            [sg.Image(source=dicc['foto'],key="avatar",pad=((90,10)),subsample=(2),s=(300,300),background_color='white')],
            [sg.Button('Cambiar Foto',button_color=("#63ABE7"),key="foto",pad=(150,0))]
        ]
        layout = [
                    [   sg.Column(columna1,fondo),
                        sg.VSeparator(color='white',pad=(100,0)),
                        sg.Column(columna2,fondo)
                    ]
                ]
        window = sg.Window('Registro',layout,background_color='white',scaling=3,margins=(100,100),resizable=True)
        #visible = True
        errores = False
        size = (500,500)
        while True:
            event,value = window.read()
            if event == "foto":
                direc = sg.PopupGetFile("Seleccione su foto",background_color=("white"),text_color=("black"),button_color=("white","green"))
                try:
                    findir = direc.split('/')
                    if ('.png' in findir[-1]):
                        window['avatar'].update(source=direc,subsample=2)
                        dicc['foto'] = direc
                        #print (findir)
                    else: 
                        sg.popup('Por favor, elija una imagen .png')
                except AttributeError:
                    sg.popup('No se encuentra foto')
            if event == "guardar":
                errores = False
                if value['nick'] == '':
                    sg.popup_ok("Complete el campo: nick",title='Error',background_color=("white"),text_color=('Black'),button_color=('green'))
                    errores = True
                if value['nombre'] == '':
                    sg.popup_ok("Complete el campo: Nombre",title='Error',background_color=("white"),text_color=('Black'),button_color=('green'))
                    errores = True
                if value['edad'] == '':
                    sg.popup_ok("Complete el campo: Edad",title='Error',background_color=("white"),text_color=('Black'),button_color=('green'))
                    errores = True
                if value['completar'] == '':
                    sg.popup_ok("Complete el campo: Genero",title='Error',background_color=("white"),text_color=('Black'),button_color=('green'))
                if errores == False:
                    print(dicc['foto'])
                    dicc = {'Nombre':value['nombre'],
                            'nick':value['nick'],
                            'Edad':value['edad'],
                            'Genero':value['completar'],
                            'foto':dicc['foto']}
                    modificar_Json(dicc)
                    #print(dicc)
                    #print (genero)
            if event == sg.WIN_CLOSED or event == 'salir':
                window.close()
                return dicc

