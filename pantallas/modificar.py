...
def main (dicc):
        ...
        columna1 = [
            ...
            #Input de nick
            [sg.Text('nick')],[sg.Input(dicc['nick'],size=(20,5),font=('calibri',10),key="nick",disabled=True)],
            #Input de nombre      
            [sg.Text('Nombre')],[sg.Input(dicc['Nombre'],size=(20,5),font=('calibri',10),key='nombre')],
            #Input de edad
            [sg.Text('Edad')],[sg.Input(dicc['Edad'],size=(8,5),key='edad')],
            #Genero
            [sg.Text('Género autopercibido')],
            [sg.Input(dicc['Genero'],size=(8,5),disabled=False,key=('completar'))],
            # Guardar los datos
        ...
        columna2 = [
            [sg.Image(source=dicc['foto'],key="avatar",pad=((90,10)),subsample=(2),s=(300,300))],
            [sg.Button('Cambiar Foto',key="foto",pad=(150,0))]
        ]
        layout = [
                    [   sg.Column(columna1),
        ...
        while True:
            ...
            if event == "foto":
                direc = sg.PopupGetFile("Seleccione su foto")
                try:
                    findir = direc.split('/')
                    if ('.png' in findir[-1]):
            ...
            if event == "guardar":
                errores = False
                if value['nick'] == '':
                    sg.popup_ok("Complete el campo: nick",title='Error')
                    errores = True
                if value['nombre'] == '':
                    sg.popup_ok("Complete el campo: Nombre",title='Error')
                    errores = True
                if value['edad'] == '':
                    sg.popup_ok("Complete el campo: Edad",title='Error')
                    errores = True
                if value['completar'] == '':
